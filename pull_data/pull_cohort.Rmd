...
medications_df <- con %>%
  tbl("SHC_med_de") %>%
  head(n = 100000) %>%
  select(start_time, end_time, order_time) %>%
  collect()

nrow( filter( medications_df, start_time > end_time ) )
...
medications_df <- con %>% 
  ...
  mutate(start_time = as.POSIXct(strptime(start_time,"%Y-%m-%d %H:%M:%S"))) %>% 
  mutate(start_time = floor_date(start_time, unit="days")) %>% 
  # mark which medications were inpatient
  mutate(inpat_med = 
           (hosp_admsn_time <= start_time & hosp_disch_time >= start_time)
         ) %>% 
  replace_na(list(inpat_med = F)) 


# write_csv(medications_df, 
#           file = "/Users/Tarun/Documents/BIO_RESEARCH/pull_data/medications_df.csv")
## 12 minutes!! 


write_medications_df  <- function(medications_df) {
  write_csv(medications_df, 
...
# Merge on the medications dictionary to find BSA's 
...
treatment_df <- med_and_dict_df %>% 
  ...
  # kill any rows from patient with any treatment which was not a treatment row
...
# CHECK: patient_id==1: start_time=="2015-02-05", medication_id==25365
# CHECK: patient_id==271815 => "2015-03-02", medication_id==96559
...
treatment_df <- treatment_df %>% 
  select(patient_id, start_time, treatment, medication_id)
...
cohort_diag_df <- treatment_df %>% 
  ...
  mutate( early_diag =
            !is.na(contact_date_time) & 
            floor_date(contact_date_time, unit = "days") <= start_time + 4*dweeks(x=1) ) %>%
  replace_na( list(early_diag=F) ) 
