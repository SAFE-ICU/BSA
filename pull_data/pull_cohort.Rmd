...
medications_df <- con %>% 
  ...
  select(patient_id, 
          ...
          order_time) %>% 
  collect() %>% 
  inner_join(first_visit_df_filt, by = c("patient_id" = "patient_id")) %>% 
  ...
  mutate(inpat_med = (hosp_admsn_time <= order_time & hosp_disch_time >= order_time))
...
med_and_dict_df <- medications_df %>% 
  ...
  select(-X1) %>% 
  arrange(patient_id) %>% select(-order_time, -medication_id, -inpat_med, -bsa )
...
treatment_df <- treatment_df %>% 
  select(patient_id, hosp_admsn_time, hosp_disch_time, treatment)
...
cohort_diag_df <- treatment_df %>% 
  ...
  mutate( contact_date_time = as.POSIXct( contact_date_time ), 
          hosp_admsn_time   = as.POSIXct( hosp_admsn_time), 
          hosp_disch_time   = as.POSIXct( hosp_disch_time ) ) %>% 
  mutate( early_diag = !is.na(contact_date_time) & 
               floor_date(contact_date_time, unit = "days") <= hosp_disch_time ) %>% 
