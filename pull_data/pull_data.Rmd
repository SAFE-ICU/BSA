...
write.csv(medications_df, file = "/Users/Tarun/Documents/BIO_RESEARCH/pull_data/medications_df.csv")
...
read.csv("/Users/Tarun/Documents/BIO_RESEARCH/pull_data/medications_df.csv")
...
med_dict_df <- con %>%
  ...
  arrange(medication_id,rxcui_str) %>% 
  collect() %>% 
  mutate(n = 1) %>% 
  spread(rxcui_str, n)
...
med_and_dict_df <- medications_df %>% 
  inner_join(med_dict_df, by = "medication_id")

dim(med_and_dict_df)
proc.time() - ptm

# figure out where event_time is coming from
# check out count and tally
```
